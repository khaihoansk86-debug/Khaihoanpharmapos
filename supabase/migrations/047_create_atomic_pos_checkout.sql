-- Atomic fast path for simple retail/ecommerce POS orders.
-- Complex flows (returns, internal issues, dose cutting, combos) keep using
-- their existing specialized services.
...
BEGIN
    ...
    -- Serialize retries of the same checkout without locking unrelated sales.
    ...
    LOOP
        ...
        IF NULLIF(v_item->>'preferred_batch_id', '') IS NOT NULL THEN
        ...
        INSERT INTO public.order_items (
            ...
            CASE WHEN v_allocation_count = 1 THEN v_first_batch_id ELSE NULL END,
    ...
    IF NULLIF(p_order->>'customer_id', '') IS NOT NULL THEN
        UPDATE public.customers
           SET total_spent = GREATEST(0, COALESCE(total_spent, 0) + COALESCE((p_order->>'total')::numeric, 0)),
