-- Migration 022_remove_auto_cashbook_sales_income.sql
-- Disable auto-recording of retail sales receipts (PT-HD...) in the cashbook.

-- 1. Update trigger function fn_auto_cashbook_order to skip retail sales income
...
BEGIN
    IF NEW.status = 'completed' AND (OLD IS NULL OR OLD.status <> 'completed') THEN
        ...
        ELSIF NEW.total < 0 THEN
            ...
            v_code := 'PC-' || NEW.order_code;
        ELSE
            -- Normal retail sales income: DO NOT record in cashbook
            RETURN NEW;
        END IF;
...
-- 2. Delete existing automatic retail sales receipts from the cashbook
DELETE FROM public.cashbook_transactions
WHERE ref_type = 'sales' 
  AND type = 'income' 
  AND category = 'Doanh thu ban le';
