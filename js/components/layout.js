 ...
 * @param {'products'|'invoices'|'inventory'|'customers'|'employees'|'purchase'|'overview'} activeTab
 */
export function initLayout(pageType = 'admin', activeTab = 'products') {
    const headerContainer = document.getElementById('app-header');
    if (headerContainer) {
        headerContainer.innerHTML = pageType === 'pos' ? renderPOSHeader() : renderAdminHeader(activeTab);
        bindLayoutEvents();
    }
    ...
    <header class="sticky top-0 z-[100] w-full bg-slate-900 text-white h-14 flex items-center justify-between px-4 transition-all duration-300">
        <div class="flex items-center gap-6 h-full">
            ...
            <nav class="flex items-center h-full gap-0.5" aria-label="Menu chính">
                ...
                ${renderTab('purchase', 'fa-cart-shopping', 'Đặt hàng', activeTab === 'purchase')}
                ${renderTab('customers', 'fa-user-group', 'Khách hàng', activeTab === 'customers')}
                ${renderTab('employees', 'fa-user-clock', 'Nhân viên', activeTab === 'employees')}
