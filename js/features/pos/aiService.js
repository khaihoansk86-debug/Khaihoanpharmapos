...
/**
 * Phân tích giỏ hàng và đưa ra gợi ý bán thêm (Cross-sell)
 */
        ...
        CROSS_SELL_RULES.forEach(rule => {
            ...
            if (match) {
                rule.suggestions.forEach(sug => {
                    ...
                    if (!alreadyInCart) {
                        // Tìm sản phẩm thực tế trong kho để lấy mã đúng
...
/**
 * Render danh sách gợi ý vào giao diện
 */
export function renderAISuggestions(suggestions) {
    ...
    container.innerHTML = suggestions.map(sug => `
        <div class="px-5 py-3 bg-white dark:bg-slate-800 rounded-xl border border-blue-100 dark:border-blue-900 shadow-sm shrink-0 flex items-center gap-3 group hover:border-blue-500 transition-all cursor-pointer" 
             onclick="window.selectProduct('${sug.realCode || sug.code}')">
            <div class="w-10 h-10 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors text-lg">
                <i class="fa-solid fa-plus"></i>
            </div>
            <div class="min-w-0">
                <div class="text-sm font-black text-slate-700 dark:text-slate-200 uppercase">${sug.name}</div>
                <div class="text-xs text-slate-500 truncate">${sug.note}</div>
            </div>
        </div>
    `).join('');
}
