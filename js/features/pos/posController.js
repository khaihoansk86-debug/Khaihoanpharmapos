...
function getProductCartKey(item) {
    ...
    const product = allProducts.find(p =>
        (idKey && normalizeKey(p.id) === idKey) ||
        (codeKey && normalizeKey(p.product_code) === codeKey)
    );

    if (product?.id != null) return `id:${normalizeKey(product.id)}`;
...
function mergeDuplicateProductRows(items) {
    ...
    items.forEach(item => {
        ...
        mergedItems[existingIndex].quantity =
            Number(mergedItems[existingIndex].quantity || 0) + Number(item.quantity || 0);
    });
...
function addProductToCart(product) {
    if (window.POS_RETURN_MODE) {
        ...
        alert('Chá»‰ cÃ³ thá»ƒ tráº£ hÃ ng náº±m trong hÃ³a Ä‘Æ¡n gá»‘c.');
        return;
    }
...
function findQuickDoseProduct(price) {
    return allProducts.find(product => {
        const baseUnit = getBaseUnit(product);
        const productText = normalizeKey(`${product.name || ''} ${product.product_code || ''}`);
        const isDoseProduct = productText.includes('LIEU') || productText.includes('LIỀU') || productText.includes('DOSE');
        return isDoseProduct && Number(baseUnit.retail_price || 0) === Number(price || 0);
    });
}

function getQuickProductCodes() {
    try {
        return JSON.parse(localStorage.getItem(QUICK_PRODUCTS_STORAGE_KEY) || '[]');
    } catch {
        return [];
    }
}

function setQuickProductCodes(productCodes) {
    const uniqueCodes = [...new Set(productCodes.map(code => String(code || '').trim()).filter(Boolean))];
    localStorage.setItem(QUICK_PRODUCTS_STORAGE_KEY, JSON.stringify(uniqueCodes));
}

function getQuickProducts() {
    const quickCodes = getQuickProductCodes().map(normalizeKey);
    return quickCodes
        .map(code => allProducts.find(product => normalizeKey(product.product_code) === code))
        .filter(Boolean);
}

function quickButtonClass(color = 'emerald') {
    if (color === 'blue') {
        return 'px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-xl border border-blue-100 dark:border-blue-800/50 font-bold text-sm hover:bg-blue-100 transition-all whitespace-nowrap';
    }
    if (color === 'slate') {
        return 'px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-sm hover:bg-slate-200 transition-all whitespace-nowrap';
    }
    return 'px-4 py-2 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-xl border border-emerald-100 dark:border-emerald-800/50 font-bold text-sm hover:bg-emerald-100 transition-all whitespace-nowrap';
}
...
function renderQuickActions() {
    ...
    const productButtons = quickProducts.map(product => {
        ...
        const label = price > 0
            ? `${product.name} ${new Intl.NumberFormat('vi-VN').format(price)}`
            : product.name;

        return `<button data-quick-product-code="${product.product_code}" class="${quickButtonClass()}">${label}</button>`;
    }).join('');

    const doseButtons = quickProducts.length > 0
        ? ''
        : DEFAULT_QUICK_DOSES.map(price => `
            <button data-quick-dose="${price}" class="${quickButtonClass()}">Liều ${(price / 1000).toLocaleString('vi-VN')}k</button>
        `).join('');

    container.innerHTML = `
        <span class="text-xs font-bold text-slate-400 uppercase whitespace-nowrap mr-2">Chọn nhanh:</span>
        ${productButtons || doseButtons}
        <button data-action="configure-quick-products" class="${quickButtonClass('blue')}">
            <i class="fa-solid fa-gear mr-1"></i> Tùy chọn
        </button>
        <button data-action="add-custom-dose" class="${quickButtonClass('slate')}">
            <i class="fa-solid fa-plus mr-1"></i> Liều khác
        </button>
    `;
}

function ensureQuickProductModal() {
    if (document.getElementById('quickProductModal')) return;

    const modal = document.createElement('div');
    modal.id = 'quickProductModal';
    modal.className = 'fixed inset-0 z-[100] hidden items-center justify-center bg-slate-900/60 p-4';
    modal.innerHTML = `
        <div class="w-full max-w-2xl rounded-2xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <h3 class="font-black text-slate-800 dark:text-white">Tùy chọn sản phẩm chọn nhanh</h3>
                <button data-action="close-quick-products" class="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-500">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <div class="p-4">
                <input id="quickProductSearch" type="text" placeholder="Tìm tên hàng hoặc mã hàng..." class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-blue-500">
                <div id="quickProductList" class="mt-4 max-h-80 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800"></div>
            </div>
            <div class="p-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                <button data-action="reset-quick-products" class="${quickButtonClass('slate')}">Dùng liều mặc định</button>
                <button data-action="save-quick-products" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-colors">Lưu chọn nhanh</button>
            </div>
        </div>
    `;
    document.body.appendChild(modal);

    document.getElementById('quickProductSearch')?.addEventListener('input', renderQuickProductPicker);
    document.getElementById('quickProductList')?.addEventListener('change', (event) => {
        if (!event.target.classList.contains('quick-product-checkbox')) return;

        const code = normalizeKey(event.target.value);
        if (!code) return;
        if (event.target.checked) {
            pendingQuickProductCodes.add(code);
        } else {
            pendingQuickProductCodes.delete(code);
        }
    });
}

function renderQuickProductPicker() {
    const list = document.getElementById('quickProductList');
    const search = normalizeKey(document.getElementById('quickProductSearch')?.value || '');
    if (!list) return;

    const matchedProducts = allProducts
        .filter(product => {
            const haystack = normalizeKey(`${product.name || ''} ${product.product_code || ''}`);
            return !search || haystack.includes(search);
        })
        .slice(0, 80);

    list.innerHTML = matchedProducts.map(product => {
        const baseUnit = getBaseUnit(product);
        const checked = pendingQuickProductCodes.has(normalizeKey(product.product_code)) ? 'checked' : '';
        return `
            <label class="flex items-center justify-between gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer">
                <div class="min-w-0">
                    <div class="font-bold text-slate-800 dark:text-slate-100 truncate">${product.name || ''}</div>
                    <div class="text-xs text-slate-500">${product.product_code || ''} - ${baseUnit.unit_name || 'N/A'} - ${new Intl.NumberFormat('vi-VN').format(baseUnit.retail_price || 0)}đ</div>
                </div>
                <input type="checkbox" class="quick-product-checkbox w-5 h-5 accent-blue-600" value="${product.product_code || ''}" ${checked}>
            </label>
        `;
    }).join('');
}
...
function openQuickProductModal() {
    ...
    pendingQuickProductCodes = new Set(getQuickProductCodes().map(normalizeKey));
    renderQuickProductPicker();
...
function saveQuickProductConfig() {
    setQuickProductCodes([...pendingQuickProductCodes]);
    renderQuickActions();
    closeQuickProductModal();
}

function resetQuickProductConfig() {
    localStorage.removeItem(QUICK_PRODUCTS_STORAGE_KEY);
    renderQuickActions();
    closeQuickProductModal();
}
...
window.updateQuantity = (id, delta) => {
    ...
    if (item) { item.quantity = Math.min(maximumQuantity, Math.max(minimumQuantity, Number(item.quantity || 0) + delta)); renderCurrentCart(); }
};
...
window.updateItemUnit = (id, unitName) => {
    ...
    if (item) {
        ...
        if (selectedUnit) { item.unit = unitName; item.price = selectedUnit.retail_price || 0; renderCurrentCart(); }
    }
};
...
window.clearCart = () => {
    ...
    if (confirm("Xóa toàn bộ giỏ hàng?")) { cart = []; renderCurrentCart(); }
};

window.addQuickDose = (price) => {
    const quickDoseProduct = findQuickDoseProduct(price);
    if (quickDoseProduct) {
        addProductToCart(quickDoseProduct);
        renderCurrentCart();
        return;
    }
...
window.processPayment = async () => {
    ...
    if (!window.POS_EDIT_MODE && payableItems.length === 0) {
        alert(window.POS_RETURN_MODE ? 'Chưa chọn sản phẩm cần trả!' : 'Giỏ hàng không có sản phẩm cần thanh toán!');
        return;
    }

    if (!window.POS_RETURN_MODE && amountReceived < total) {
        alert('Tiền khách đưa chưa đủ!');
        return;
    }

    const btn = document.querySelector('[data-action="process-payment"]');
    if (btn) { btn.disabled = true; btn.textContent = window.POS_RETURN_MODE ? 'Đang trả hàng...' : (window.POS_EDIT_MODE ? 'Đang cập nhật...' : 'Đang lưu...'); }
    ...
    try {
        ...
        const order = window.POS_RETURN_MODE
            ? await createReturnOrder(returnOrder, orderPayload, cart)
            : (window.POS_EDIT_MODE
        ...
        if (btn) { btn.disabled = false; btn.textContent = window.POS_RETURN_MODE ? 'Hoàn tất trả hàng' : (window.POS_EDIT_MODE ? 'Cập nhật hóa đơn' : 'Thanh toán'); }
    }
};
...
function setupPOSEventListeners() {
    document.addEventListener('click', (event) => {
        ...
        if (quickProductButton) {
            window.selectProduct(quickProductButton.dataset.quickProductCode);
            return;
        }
...
function applyReturnModeUI(order) {
    const banner = document.getElementById('posEditModeBanner');
    const title = document.getElementById('posEditModeTitle');
    if (banner) {
        if (title) title.textContent = `Dang tra hang hoa don ${order.order_code}`;
        banner.classList.remove('hidden', 'bg-amber-600', 'border-amber-500', 'shadow-amber-500/20');
        banner.classList.add('bg-emerald-600', 'border-emerald-500', 'shadow-emerald-500/20');
    }

    const paymentButton = document.querySelector('[data-action="process-payment"]');
    if (paymentButton) {
        paymentButton.classList.remove('bg-blue-600', 'hover:bg-blue-700', 'shadow-blue-500/30');
        paymentButton.classList.add('bg-emerald-600', 'hover:bg-emerald-700', 'shadow-emerald-500/30');
        paymentButton.innerHTML = `
            <div class="absolute inset-0 bg-gradient-to-r from-emerald-300/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"></div>
            <span class="text-[10px] uppercase font-bold opacity-70 mb-0.5">Tráº£ hÃ ng</span>
            <div class="flex items-center gap-2">
                <i class="fa-solid fa-rotate-left"></i> HOÃ€N Táº¤T TRáº¢
            </div>
        `;
    }
...
function buildCartFromOrder(order) {
    const orderCart = (order.items || []).map((item) => {
        const product = allProducts.find(p =>
            normalizeKey(p.id) === normalizeKey(item.product_id) ||
            normalizeKey(p.product_code) === normalizeKey(item.product_code)
        );
...
function buildReturnCartFromOrder(order) {
    return (order.items || [])
        ...
        .map((item) => {
            ...
            return {
                lineId: item.product_id || `return-item-${item.id}`,
                id: item.product_id || null,
                batchId: item.batch_id || null,
                code: item.product_code || 'MANUAL',
                name: item.product_name,
...
async function loadOrderForReturn() {
    ...
    try {
        returnOrder = await fetchOrderDetail(returnOrderId);
        if (returnOrder.status === 'cancelled') {
            alert('KhÃ´ng thá»ƒ tráº£ hÃ ng tá»« hÃ³a Ä‘Æ¡n Ä‘Ã£ há»§y.');
            window.location.href = 'invoices.html';
            return;
        }

        cart = buildReturnCartFromOrder(returnOrder);
        document.getElementById('customerName').value = returnOrder.customer_name || '';
        document.getElementById('customerPhone').value = returnOrder.customer_phone || '';
        document.getElementById('discountAmount').value = 0;
        document.getElementById('orderNote').value = `Tra hang tu hoa don ${returnOrder.order_code}`;

        applyReturnModeUI(returnOrder);
        renderCurrentCart();
    } catch (err) {
        console.error('Lá»—i táº£i hÃ³a Ä‘Æ¡n Ä‘á»ƒ tráº£ hÃ ng:', err);
        alert('KhÃ´ng thá»ƒ táº£i hÃ³a Ä‘Æ¡n Ä‘á»ƒ tráº£ hÃ ng: ' + err.message);
        window.location.href = 'invoices.html';
    }
}
