...
const replacement = `        const prefix = 'PT-TN';
        const rand = Math.floor(1000 + Math.random() * 9000);
        const txCode = \`\${prefix}-\\\\\$\` + \`{order.order_code}-\\\\$\` + \`{rand}\`; // Wait, let's just use standard template literal:
        // txCode = \`\${prefix}-\${order.order_code}-\${rand}\`
        // Since we are writing this as a JS string template, we need to escape the backticks and the \${}
        // Let's write the string dynamically:
        // const txCode = \`\${prefix}-\${order.order_code}-\${rand}\`;
`;

// To avoid template literal escaping issues inside the script itself, I can write the replacement string like this:
